...
function ParsePhotoInfo(
  ...
  if (description) {
    photoInfo.photographer.push(description.replace(/Photographer: /g, ""));
  }
  ...
  for (const tag of tags) {
    ...
    if (tag.startsWith(TAG_EVENT)) {
      photoInfo.event.push(tag.replace(new RegExp(TAG_EVENT + "\\$", "g"), ""));
      continue;
    }
    if (tag.startsWith(TAG_TEAM)) {
      photoInfo.team.push(tag.replace(new RegExp(TAG_TEAM + "\\$", "g"), ""));
      continue;
    }
    if (tag.startsWith(TAG_PERSON)) {
      photoInfo.person.push({
        name: tag.replace(new RegExp(TAG_PERSON + "\\$", "g"), ""),
      });
      continue;
    }
    if (tag.startsWith(TAG_ALBUM)) {
      photoInfo.album.push(tag.replace(new RegExp(TAG_ALBUM + "\\$", "g"), ""));
      continue;
    }
    if (tag.startsWith(TAG_PHOTOGRAPHER)) {
      photoInfo.photographer.push(
        tag.replace(new RegExp(TAG_PHOTOGRAPHER + "\\$", "g"), ""),
      );
      continue;
    }
...
function SerializePhotoInfo(photoInfo: PhotoInfo | null | undefined): string[] {
  if (!photoInfo) {
    return [];
  }
  const tags: string[] = [];
