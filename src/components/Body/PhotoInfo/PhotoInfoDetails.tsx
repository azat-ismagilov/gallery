...
import { Person, usePhotoInfo } from "./PhotoInfoContext";

const ComaSeparated = (list: React.ReactNode[]): React.ReactNode[] => {
...
const PersonInfo: React.FC<PersonInfoProps> = ({ setFace }) => {
  const { photoInfo } = usePhotoInfo();
...
const PersonEdit: React.FC = () => {
  ...
  return (
    <Autocomplete<string, true, false, true>
      ...
      onChange={(event, newValue) => {
        setPerson(newValue.map((name) => ({ name })));
      }}
      fullWidth={false}
      renderInput={(params) => (
        <TextField
